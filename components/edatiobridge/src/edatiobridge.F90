...
module edatiobridge_mod
  ...
  use io_server_client_mod, only : COMMAND_TAG, DATA_TAG, REGISTER_COMMAND, DEREGISTER_COMMAND, DATA_COMMAND_START, &
       ...
       build_mpi_type_field_description, build_mpi_type_definition_description, populate_type_extents, &
       pack_scalar_field, pack_array_field, pack_map_field, send_data_as_edat_event
  ...
  logical :: io_server_enabled, in_finalisation_callback
	double precision :: begin_t
	integer :: num_its

  public edatiobridge_get_descriptor

contains

  type(component_descriptor_type) function edatiobridge_get_descriptor()
    edatiobridge_get_descriptor%name="edatiobridge"
    edatiobridge_get_descriptor%version=0.1
    edatiobridge_get_descriptor%initialisation=>init_callback
    edatiobridge_get_descriptor%timestep=>timestep_callback
    edatiobridge_get_descriptor%finalisation=>finalisation_callback
  end function edatiobridge_get_descriptor

  !> Initialisation call back, called at the start of the model run
  !! @param current_state The current model state
  subroutine init_callback(current_state)
    type(model_state_type), target, intent(inout) :: current_state
		character(len=STRING_LENGTH) :: reps
    integer :: mpi_type_definition_description, mpi_type_field_description, ierr, status

    call get_environment_variable("MONC_BENCHMARK_NUMS", reps, status=status)

		if (status == 0) then
			num_its=conv_to_integer(reps)
		else
			num_its=1
		end if
  ...
  subroutine timestep_callback(current_state)
    ...
    integer :: i, j, io_server, ierr
		double precision :: start

    if (.not. io_server_enabled) return

    io_server=current_state%parallel%corresponding_io_server_process
    ...
    do j=1, num_its
		!j=0
		!start=MPI_Wtime()
		!do while (MPI_Wtime() - start .lt. 5)
      if (j .gt. 1) current_state%timestep=current_state%timestep+1
			!j=j+1
      do i=1, size(data_definitions)
        if (data_definitions(i)%frequency .gt. 0) then
          if (mod(current_state%timestep, data_definitions(i)%frequency) == 0) then
            ...
            call send_data_as_edat_event(EDAT_INT, 1, io_server, "sendreq", int_to_send=i)
            call send_data_as_edat_event(EDAT_BYTE, size(data_definitions(i)%send_buffer), &
              io_server, "data", char_array_to_send=data_definitions(i)%send_buffer)
						!call MPI_Recv(0, 0, MPI_INT, io_server, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
          end if
        end if
      end do
    end do
  ...
  subroutine finalisation_callback(current_state)
    ...
    call send_data_as_edat_event(EDAT_NONE, 0, current_state%parallel%corresponding_io_server_process, "deregister")
  end subroutine finalisation_callback

  !> Builds the MPI data types that correspond to the field descriptions and sizings
  subroutine allocate_data_buffers()
    integer :: i, dump_send_buffer_size

    do i=1, size(data_definitions)
      dump_send_buffer_size=determine_buffer_size_for_definition(data_definitions(i))
  ...
  integer function determine_buffer_size_for_definition(specific_data_definition)
    type(io_configuration_data_definition_type), intent(inout) :: specific_data_definition

    integer :: type_extents(5), i, j, field_start, data_type, field_array_sizes, temp_size, field_ignores, buffer_size
    logical :: field_found
    type(io_server_sendable_field_sizing) :: field_size_info
    character(len=STRING_LENGTH) :: orig_field_name

    type_extents=populate_type_extents()
    ...
    do i=1, specific_data_definition%number_of_data_fields
      if (data_type == 0) then
        ...
        if (data_type .ne. specific_data_definition%fields(i)%data_type) then
          buffer_size=buffer_size+(type_extents(data_type) * (((i-1-field_ignores)-field_start) + 1 + field_array_sizes))
          field_start=i
          field_array_sizes=0
          field_ignores=0
          data_type=specific_data_definition%fields(i)%data_type
      ...
      if (specific_data_definition%fields(i)%field_type .eq. ARRAY_FIELD_TYPE .or. &
        ...
        if (index(specific_data_definition%fields(i)%name, "~") == 0) then
          orig_field_name=specific_data_definition%fields(i)%name
        else
          orig_field_name=specific_data_definition%fields(i)%name(:scan(specific_data_definition%fields(i)%name, "~")-1)
        end if
        field_size_info=get_sendable_field_sizing(orig_field_name, field_found)
    ...
    if (field_start .le. i-1) then
      buffer_size=buffer_size+(type_extents(data_type) * (((i-1)-field_start) + 1 + field_array_sizes))
    end if
    determine_buffer_size_for_definition=buffer_size
  end function determine_buffer_size_for_definition
  ...
  subroutine send_monc_specific_data_to_server(current_state)
    type(model_state_type), target, intent(inout) :: current_state

    type(data_sizing_description_type), dimension(:), allocatable :: data_description
    character, dimension(:), allocatable :: buffer
    integer :: number_unique_fields, buffer_size
    ...
    call send_data_field_sizes_to_server(current_state, data_description, number_unique_fields)
    buffer_size=(kind(dreal)*current_state%local_grid%size(Z_INDEX)) + (STRING_LENGTH * current_state%number_q_fields)
    allocate(buffer(buffer_size))
    call send_general_monc_information_to_server(current_state, buffer)
  ...
  subroutine send_data_field_sizes_to_server(current_state, data_description, number_unique_fields)
    type(model_state_type), target, intent(inout) :: current_state
    ...
    character(len=STRING_LENGTH) :: field_name, orig_field_name
    ...
    do i=1, number_unique_fields
      ...
      if (index(field_name, "~") == 0) then
        orig_field_name=field_name
      else
        orig_field_name=field_name(:scan(field_name, "~")-1)
      end if
      if (c_contains(sendable_fields, orig_field_name)) then
        call assemble_individual_description(data_description, next_index, field_name, get_sendable_field_sizing(orig_field_name))
        next_index=next_index+1
      end if
    end do

    call send_data_as_edat_event(EDAT_BYTE, int(sizeof(data_description)), &
      current_state%parallel%corresponding_io_server_process, &
      "fieldsizes", data_sizing_to_send=data_description)
  ...
  subroutine send_general_monc_information_to_server(current_state, buffer)
    type(model_state_type), target, intent(inout) :: current_state
    ...
    call send_data_as_edat_event(EDAT_BYTE, current_loc-1, current_state%parallel%corresponding_io_server_process, &
      "moncinfo", char_array_to_send=buffer)
  end subroutine send_general_monc_information_to_server
  ...
  subroutine register_with_io_server(current_state, mpi_type_definition_description, mpi_type_field_description)
    ...
    call send_data_as_edat_event(EDAT_NONE, 0, current_state%parallel%corresponding_io_server_process, "register")
  ...
  integer function pack_scalar_into_send_buffer(current_state, data_definition, field, current_buffer_point)
    ...
    if (index(field%name, "~") == 0) then
      orig_field_name=field%name
    else
      orig_field_name=field%name(:scan(field%name, "~")-1)
    end if
    ...
    else if (orig_field_name .eq. "timestamp") then
       pack_scalar_into_send_buffer=pack_scalar_field(data_definition%send_buffer, current_buffer_point, &
            real_value=begin_t)
    else
  ...
  integer function handle_component_field_scalar_packing_into_send_buffer(current_state, data_definition, &
    ...
    field_descriptor=get_component_field_descriptor(orig_field_name)
    published_value=get_component_field_value(current_state, orig_field_name)
