...
module diagnostic_federator_mod
  ...
  use conversions_mod, only : conv_to_string, conv_to_real, conv_to_integer
  use reduction_inter_io_mod, only : init_reduction_inter_io, finalise_reduction_inter_io, &
       perform_inter_io_reduction, perform_inter_io_allreduction, get_reduction_operator
  ...
  use data_utils_mod, only : get_scalar_integer_from_monc, get_scalar_real_from_monc, is_field_present, &
       ...
       get_array_double_from_monc, get_array_integer_from_monc, get_scalar_logical_from_monc, check_thread_status
  ...
  use writer_field_manager_mod, only : provide_field_to_writer_federator, writer_task_data
 ...
 public initialise_diagnostic_federator, finalise_diagnostic_federator, &
      pass_fields_to_diagnostics_federator, determine_diagnostics_fields_available, diagnostics_federator_task
contains
  ...
  subroutine diagnostics_federator_task(events, number_events)
    ...
    integer :: hash_loc, underscore_loc

    call getEvents(events, number_events,  processed_events)

    hash_loc=scan(processed_events(1)%metadata%event_id, "#")
    underscore_loc=scan(processed_events(1)%metadata%event_id, "_")

    call pass_fields_to_diagnostics_federator(this_io_configuration, &
      conv_to_integer(processed_events(1)%metadata%event_id(:hash_loc-1)), &
      conv_to_integer(processed_events(1)%metadata%event_id(hash_loc+1:underscore_loc-1)), &
      processed_events(1)%byte_data)
  end subroutine diagnostics_federator_task
  ...
  subroutine pass_fields_to_diagnostics_federator(io_configuration, source, data_id, data_dump)
    ...
    character, dimension(:), pointer, intent(in) :: data_dump
  ...
  subroutine handle_diagnostic_calculation_completed(io_configuration, diagnostic_index, timestep_entry, diagnostics_by_timestep)
    ...
    type(writer_task_data) :: wtd
    character :: raw_d(sizeof(wtd))
    ...
    if (diagnostic_definitions(diagnostic_index)%collective) then
      wtd%field_name=diagnostic_definitions(diagnostic_index)%diagnostic_name
      wtd%namespace=diagnostic_definitions(diagnostic_index)%diagnostic_namespace
      wtd%timestep=timestep_entry%timestep
      wtd%frequency=diagnostic_definitions(diagnostic_index)%generation_timestep_frequency
      wtd%source=timestep_entry%source
      wtd%time=timestep_entry%time
      wtd%values=>diagnostics_value_entry%values

      raw_d=transfer(wtd, raw_d)
      call edatFireEvent(raw_d, EDAT_BYTE, size(raw_d), EDAT_SELF, "writer_from_diag")
  ...
  subroutine perform_inter_io_communication(io_configuration, timestep_entry, all_entries_at_timestep, &
    ...
    if (activity%activity_type == REDUCTION_TYPE) then
      ...
      call perform_inter_io_allreduction(io_configuration, value_to_send%values, size(value_to_send%values), &
           communication_field_name, activity%communication_operator, timestep_entry%timestep, handle_completion)
