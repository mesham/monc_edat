...
module writer_field_manager_mod
  ...
  use data_utils_mod, only : get_scalar_integer_from_monc, get_scalar_real_from_monc, get_scalar_logical_from_monc, &
       ...
       unpack_scalar_integer_from_bytedata, unpack_scalar_dp_real_from_bytedata, check_thread_status
  ...
  type writer_task_data
    character(len=60) :: field_name, namespace
    integer :: timestep, frequency, source
    real(kind=DEFAULT_PRECISION) :: time
    real(kind=DEFAULT_PRECISION), dimension(:), pointer :: values
  end type
  ...
  public initialise_writer_field_manager, finalise_writer_field_manager, provide_monc_data_to_writer_federator, &
       provide_field_to_writer_federator, is_write_field_manager_up_to_date, writer_federator_task_from_monc, &
       writer_task_data
contains
  ...
  subroutine initialise_writer_field_manager(io_configuration, continuation_run)
    ...
    call edatSubmitPersistentTask(writer_federator_task, 1, EDAT_SELF, "writer_from_diag")
  ...
  subroutine writer_federator_task_from_monc(events, number_events)
    ...
    call provide_monc_data_to_writer_federator(this_io_configuration, &
  ...
  end subroutine writer_federator_task_from_monc

  subroutine writer_federator_task(events, number_events)
	  type(c_ptr), intent(in), target :: events
    integer(c_int), value, intent(in) :: number_events
    ...
    wtd=transfer(processed_events(1)%byte_data, wtd)

    call provide_field_to_writer_federator_rvalues_src(this_io_configuration, wtd%field_name, wtd%namespace, &
       wtd%values, wtd%timestep, wtd%time, wtd%frequency, -1) !wtd%source)
    if (associated(wtd%values)) deallocate(wtd%values)
  end subroutine writer_federator_task
  ...
  subroutine provide_field_to_writer_federator_src(io_configuration, field_name, field_namespace, &
    ...
    ! MAke this a task (need to package the arguments) - can we speed this up by using a user derived type with pointer to avoid large memory copy?
    ! If so ensure don't drop the data values type as it is now owned by this task (but that's probably OK, just check the provenence of this.)
