...
module writer_federator_mod
  ...
  use configuration_parser_mod, only : TIME_AVERAGED_TYPE, INSTANTANEOUS_TYPE, NONE_TYPE, GROUP_TYPE, FIELD_TYPE, IO_STATE_TYPE, &
       io_configuration_type, io_configuration_field_type, io_configuration_diagnostic_field_type, BENCHMARK_TYPE, &
  ...
  use benchmark_time_manipulation_mod, only : is_benchmark_time_manipulation_ready_to_write, perform_benchmark_time_manipulation, &
  ...
  use data_utils_mod, only : get_scalar_integer_from_monc, get_scalar_real_from_monc, get_scalar_logical_from_monc, &
       is_field_present, check_thread_status
...
contains
  ...
  subroutine initialise_writer_federator(io_configuration, diagnostic_generation_frequency, continuation_run)
    ...
    call init_instantaneous_manipulation()
		call initialise_benchmark_time_manipulation(io_configuration%my_io_rank, io_configuration%io_communicator)
  ...
  subroutine finalise_writer_federator()
    ...
    call finalise_benchmark_time_manipulation()
  ...
  subroutine close_diagnostics_file(io_configuration, writer_entry, timestep, time)
    ...
    call edatSubmitTask(handle_close_diagnostics_globalcallback, 1, EDAT_ALL, &
      trim(writer_entry%filename)//"#"//trim(conv_to_string(timestep)))
    call edatFireEvent(EDAT_NOTYPE, 0, EDAT_ALL, trim(writer_entry%filename)//"#"//trim(conv_to_string(timestep)))
  ...
  subroutine handle_close_diagnostics_globalcallback(events, number_events)
    ...
    file_name=processed_events(1)%metadata%event_id(:hash_loc-1)
    timestep=conv_to_integer(trim(processed_events(1)%metadata%event_id(hash_loc+1:)))

    writer_entry=>get_writer_entry_from_netcdf(file_name, timestep, terminated)
    ...
    do i=1, size(writer_entry%contents)
      if (writer_entry%contents(i)%enabled .and. writer_entry%contents(i)%collective_write .and. &
        ...
        call write_variable(this_io_configuration, writer_entry%contents(i), writer_entry%filename, &
          timestep, writer_entry%write_time)
    ...
    if (writer_entry%contains_io_status_dump) then
      ...
      call check_thread_status(forthread_rwlock_rdlock(time_points_rwlock))
      call store_io_server_state(this_io_configuration, writer_entries, time_points, writer_entry, timestep)
      call check_thread_status(forthread_rwlock_unlock(time_points_rwlock))
    end if

    call close_netcdf_file(this_io_configuration, file_name, timestep)

    done_chain_run=.false.
    do i=1, size(writer_entries)
      if (writer_entries(i)%filename .ne. writer_entry%filename) then
        done_chain_run=check_for_and_issue_chain_write(this_io_configuration, writer_entries(i))
        if (done_chain_run) exit
      end if
    end do
    if (.not. done_chain_run) done_chain_run=check_for_and_issue_chain_write(this_io_configuration, writer_entry)
  ...
  subroutine add_specific_field_to_writer_entry(io_configuration, writer_entry_index, io_config_facet_index, &
    ...
    if (io_configuration%file_writers(writer_entry_index)%contents(io_config_facet_index)%time_manipulation_type == &
      ...
      writer_entries(writer_entry_index)%contents(my_facet_index)%ready_to_write=>is_none_time_manipulation_ready_to_write
         ...
         BENCHMARK_TYPE) then
      writer_entries(writer_entry_index)%contents(my_facet_index)%time_manipulation=>perform_benchmark_time_manipulation
      writer_entries(writer_entry_index)%contents(my_facet_index)%ready_to_write=>is_benchmark_time_manipulation_ready_to_write
    end if
