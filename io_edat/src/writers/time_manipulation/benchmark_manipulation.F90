module benchmark_time_manipulation_mod
  ...
  integer, volatile :: tot_vals=0, latency_count=0, io_rank, communicator
...
contains
  ...
  subroutine finalise_benchmark_time_manipulation()
		...
		if (io_rank == 0) then
      print *, "Bandwidth: ", g_timediff, global_vals
      print *, "Average Latency: ", global_latency_avg/global_latency_count, " based on ", global_latency_count, " items"
      print *, "Minimum Latency: ", global_latency_min
      print *, "Maximum Latency: ", global_latency_max
    end if
  ...
  type(data_values_type) function perform_benchmark_time_manipulation(instant_values, output_frequency, &
    ...
    integer, intent(in) :: timestep
		...
		call edatLock("benchmark")

    if (index(field_name, "timestamp") .gt. 0) then
			timediff=MPI_Wtime()-instant_values(1)
      latency_avg=latency_avg+timediff
      latency_count=latency_count+1
			if (latency_min .gt. timediff) latency_min=timediff
			if (latency_max .lt. timediff) latency_max=timediff
    end if
